...
.get(cors.cors, (req, res, next) => {
    ...
    .then((favourites) => {
        if (favourites) {
            user_favourites = favourites.filter(fav => fav.user._id.toString() === req.id.toString())[0];
            if(!user_favourites) {
...
.put(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
    res.statusCode = 403;
    res.end('PUT operation is not supportes on /favourites!');
})
    ...
    (req, res, next) => {
        Favourites.find({})
            ...
            .then((favourites) => {
                ...
                if(!user.dishes.find((d_id) => {
                    if(d_id._id)
                        return d_id._id.toString() === req.params.dish.toString();
                }))
